...
export class Transport extends TransportBase {
    ...
    loadConfig(configuration) {
        ...
        const skeleton = {}; // Fill the value of the configuration_skeleton
        ...
        const returnConfiguration = Object.defineProperties({}, skeleton);
            ...
            optional: {
                ...
                wsServers: (wsServers) => {
                    ...
                    for (const wsServer of wsServers) {
                        ...
                        const url = Grammar.parse(wsServer.wsUri, "absoluteURI");
                        if (url === -1) {
                            return;
                        }
                ...
                keepAliveInterval: (keepAliveInterval) => {
                    if (Utils.isDecimal(keepAliveInterval)) {
                        const value = Number(keepAliveInterval);
                        if (value > 0) {
                            return value;
                        }
                    }
                },
                keepAliveDebounce: (keepAliveDebounce) => {
                ...
                maxReconnectionAttempts: (maxReconnectionAttempts) => {
                    if (Utils.isDecimal(maxReconnectionAttempts)) {
                        const value = Number(maxReconnectionAttempts);
                        if (value >= 0) {
                            return value;
                        }
                    }
                ...
                reconnectionTimeout: (reconnectionTimeout) => {
                    if (Utils.isDecimal(reconnectionTimeout)) {
                        const value = Number(reconnectionTimeout);
                        if (value > 0) {
                            return value;
                        }
                    }
