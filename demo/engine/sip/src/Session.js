...
import { Exception, fromBodyLegacy, getBody, Grammar, SessionState, SignalingState, Timers } from "./core";
...
export class Session extends EventEmitter {
    constructor(sessionDescriptionHandlerFactory) {
        super();
        this.data = {};
    ...
    dtmf(tones, options = {}) {
        ...
        let dtmfType = this.ua.configuration.dtmfType;
        if (this.sessionDescriptionHandler && dtmfType === C.dtmfType.RTP) {
            const sent = this.sessionDescriptionHandler.sendDtmf(tones, options);
            if (!sent) {
        ...
        if (dtmfType === C.dtmfType.INFO) {
            ...
            while (tonesArray.length > 0) {
                dtmfs.push(new DTMF(this, tonesArray.shift(), options));
            }
            if (this.tones) {
    ...
    sendRequest(method, options = {}) {
        ...
        let delegate;
        const callback = options.receiveResponse;
        if (callback) {
            delegate = {
                onAccept: (response) => callback(response.message),
                onProgress: (response) => callback(response.message),
                onRedirect: (response) => callback(response.message),
                onReject: (response) => callback(response.message),
                onTrying: (response) => callback(response.message)
            };
        }
        let request;
    ...
    onAck(incomingRequest) {
        ...
        if (this.status === SessionStatus.STATUS_WAITING_FOR_ACK) {
            if (this.sessionDescriptionHandler &&
                ...
                this.hasAnswer = true;
                this.sessionDescriptionHandler.setDescription(incomingRequest.message.body, this.sessionDescriptionHandlerOptions, this.modifiers).catch((e) => {
                    this.logger.warn(e);
                    this.terminate({
                        statusCode: "488",
        ...
        switch (incomingRequest.message.method) { // TODO: This needs a default case
            ...
            case C.INFO:
                if (this.status === SessionStatus.STATUS_CONFIRMED || this.status === SessionStatus.STATUS_WAITING_FOR_ACK) {
                    ...
                    const contentType = incomingRequest.message.getHeader("content-type");
                    if (contentType) {
                        if (contentType.match(/^application\/dtmf-relay/i)) {
                            if (incomingRequest.message.body) {
                                const body = incomingRequest.message.body.split("\r\n", 2);
                                if (body.length === 2) {
                                    let tone;
                                    let duration;
            ...
            case C.REFER:
                if (this.status === SessionStatus.STATUS_CONFIRMED) {
                    ...
                    if (this.listeners("referRequested").length) {
                        ...
                        const options = { followRefer: true };
    ...
    receiveReinvite(incomingRequest) {
        ...
        if (incomingRequest.message.hasHeader("P-Asserted-Identity")) {
            this.assertedIdentity =
                Grammar.nameAddrHeaderParse(incomingRequest.message.getHeader("P-Asserted-Identity"));
        }
        let promise;
        if (!this.sessionDescriptionHandler) {
...
export class InviteServerContext extends Session {
    constructor(ua, incomingInviteRequest) {
        ...
        const contentDisp = request.parseHeader("Content-Disposition");
        if (contentDisp && contentDisp.type === "render") {
        ...
        const set100rel = (header, relSetting) => {
    ...
    accept(options = {}) {
        ...
        this._accept(options)
            ...
            session.delegate = {
                onAck: (ackRequest) => this.onAck(ackRequest),
                onAckTimeout: () => this.onAckTimeout(),
                onBye: (byeRequest) => this.receiveRequest(byeRequest),
                onInfo: (infoRequest) => this.receiveRequest(infoRequest),
                onInvite: (inviteRequest) => this.receiveRequest(inviteRequest),
                onNotify: (notifyRequest) => this.receiveRequest(notifyRequest),
                onPrack: (prackRequest) => this.receiveRequest(prackRequest),
                onRefer: (referRequest) => this.receiveRequest(referRequest)
            };
                ...
                if (this.status === SessionStatus.STATUS_WAITING_FOR_PRACK ||
                    ...
                    if (!this.hasAnswer) {
                        ...
                        if (this.sessionDescriptionHandler.hasDescription(incomingRequest.message.getHeader("Content-Type") || "")) {
                            this.hasAnswer = true;
                            this.sessionDescriptionHandler.setDescription(incomingRequest.message.body, this.sessionDescriptionHandlerOptions, this.modifiers).then(() => {
    ...
    generateResponseOfferAnswer(options) {
        if (!this.session) {
     ...
     *                                 Note that the transaction state can change while this call is in progress.
     */
    _reliableProgressWaitForPrack(options = {}) {
        const statusCode = options.statusCode || 183;
        const reasonPhrase = options.reasonPhrase;
        const extraHeaders = (options.extraHeaders || []).slice();
        extraHeaders.push("Require: 100rel");
...
export class InviteClientContext extends Session {
    constructor(ua, target, options = {}, modifiers = []) {
        if (!ua.configuration.sessionDescriptionHandlerFactory) {
            ua.logger.warn("Can't build ISC without SDH Factory");
    ...
    cancel(options = {}) {
        ...
        const cancelReason = Utils.getCancelReason(options.statusCode, options.reasonPhrase);
    ...
    sendInvite() {
        ...
        this.outgoingInviteRequest = this.ua.userAgentCore.invite(this.request, {
            onAccept: (inviteResponse) => this.onAccept(inviteResponse),
            onProgress: (inviteResponse) => this.onProgress(inviteResponse),
            onRedirect: (inviteResponse) => this.onRedirect(inviteResponse),
            onReject: (inviteResponse) => this.onReject(inviteResponse),
            onTrying: (inviteResponse) => this.onTrying(inviteResponse)
        });
    }
    ackAndBye(inviteResponse, session, statusCode, reasonPhrase) {
    ...
    onAccept(inviteResponse) {
        ...
        switch (session.signalingState) {
            ...
            case SignalingState.HaveRemoteOffer:
                ...
                const sdh = this.sessionDescriptionHandlerFactory(this, this.ua.configuration.sessionDescriptionHandlerFactoryOptions || {});
    ...
    onRedirect(inviteResponse) {
        ...
        const cause = Utils.sipErrorCause(statusCode || 0);
